 ...
 */
export class SeedPhb2014Backgrounds1776700000000
  ...
  public async up(queryRunner: QueryRunner): Promise<void> {
    ...
    for (const bg of this.backgrounds) {
      ...
      for (const skillSlug of bg.skills) {
        ...
        if (!profId) {
          console.warn(`[SeedPhb2014Backgrounds] Proficiency ${skillSlug} not found — skipping`);
          continue;
        }
        await queryRunner.query(
      ...
      if (bg.toolProficiencies) {
        for (const toolSlug of bg.toolProficiencies) {
          ...
          if (!profId) {
            console.warn(`[SeedPhb2014Backgrounds] Tool proficiency ${toolSlug} not found — skipping`);
            continue;
          }
          await queryRunner.query(
            `INSERT INTO background_proficiencies (background_id, proficiency_id)
