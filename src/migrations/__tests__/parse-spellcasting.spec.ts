...
describe("parseSpellcastingFromSpecialAbilities", () => {
  it("returns null when special_abilities is empty or missing", () => {
    ...
    expect(
      parseSpellcastingFromSpecialAbilities([{ name: "Amphibious" }]),
    ).toBeNull();
  });
  ...
  it("parses Archmage standard spellcasting block", () => {
    const abilities = [
      {
        name: "Spellcasting",
        desc: `The archmage is an 18th-level spellcaster. Its spellcasting ability is Intelligence (spell save DC 18, +10 to hit with spell attacks). The archmage has the following wizard spells prepared:
    ...
    const abilities = [
      {
        ...
        desc: `The dragon's innate spellcasting ability is Charisma (spell save DC 21). It can innately cast the following spells, requiring no material components:
...
1/day each: dominate person`,
  ...
  it("prefers innate block when both are present", () => {
    const abilities = [
      {
        name: "Spellcasting",
        desc: `This creature is a 5th-level spellcaster. Its spellcasting ability is Wisdom (spell save DC 14, +6 to hit with spell attacks). It has the following spells prepared:
Cantrips (at will): guidance
1st level (3 slots): cure wounds`,
      },
      {
        name: "Innate Spellcasting",
        desc: `The creature's innate spellcasting ability is Charisma (spell save DC 15). It can innately cast the following spells:
At will: minor illusion`,
  ...
  it('defaults attackBonus to saveDc - 8 when "+X to hit" missing', () => {
    const abilities = [
      {
        name: "Innate Spellcasting",
        desc: `The creature's innate spellcasting ability is Charisma (spell save DC 15). It can innately cast:
At will: minor illusion`,
      },
    ];
    const res = parseSpellcastingFromSpecialAbilities(abilities);
  ...
  describe("slugifySpellName", () => {
    it("normalizes basic names", () => {
      ...
      expect(slugifySpellName("Tasha's Hideous Laughter")).toBe(
        "tasha-s-hideous-laughter",
      );
    });

    it("strips parentheticals", () => {
      expect(slugifySpellName("Meld into Stone (cast before combat)")).toBe(
        "meld-into-stone",
      );
    });
  });
