...
describe('classifyFeatureForActions', () => {
  ...
  describe('hide — padrões universais (regex)', () => {
    ...
    it('qualquer *-ability-score-improvement-* é passive', () => {
      expect(classifyFeatureForActions('wizard-ability-score-improvement-3')?.kind).toBe('hide');
      expect(classifyFeatureForActions('ability-score-improvement-fighter-6')?.kind).toBe('hide');
      expect(classifyFeatureForActions('rogue-ability-score-improvement-4')?.kind).toBe('hide');
    });

    it('qualquer epic-boon-* é passive (L19 grant)', () => {
      expect(classifyFeatureForActions('epic-boon-cleric-19')?.kind).toBe('hide');
      expect(classifyFeatureForActions('epic-boon-paladin-19')?.kind).toBe('hide');
    });

    it('qualquer subclass-feature-*-{N} é marker (informational)', () => {
      expect(classifyFeatureForActions('subclass-feature-cleric-3')?.kind).toBe('hide');
      expect(classifyFeatureForActions('subclass-feature-wizard-14')?.kind).toBe('hide');
    });

    it('qualquer *-college-improvement-* ou *-circle-improvement-* é marker', () => {
      expect(classifyFeatureForActions('wizard-school-improvement-1')?.kind).toBe('hide');
      expect(classifyFeatureForActions('cleric-domain-improvement-2')?.kind).toBe('hide');
    });

    it('spellcasting base grants são passive', () => {
      expect(classifyFeatureForActions('spellcasting-wizard-1')?.kind).toBe('hide');
      expect(classifyFeatureForActions('spellcasting-cleric-1')?.kind).toBe('hide');
      expect(classifyFeatureForActions('pact-magic-warlock-1')?.kind).toBe('hide');
    });
  });
