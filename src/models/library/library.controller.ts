...
export class LibraryController {
  ...
  async getBeasts(
    ...
    const numerator = maxCrNumeratorRaw != null ? parseInt(maxCrNumeratorRaw, 10) : NaN;
    const denominator = maxCrDenominatorRaw != null ? parseInt(maxCrDenominatorRaw, 10) : 1;
    ...
    const excludeBurrow = excludeBurrowRaw === 'true' || excludeBurrowRaw === '1';

    const monsters = await this.libraryService.findBeastsForForm(maxCr);
    const filtered = monsters
      .filter((m) => passesLocomotionFilter(m, { excludeFly, excludeSwim, excludeBurrow }))
      .map(toBeastSummary)
