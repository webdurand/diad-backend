...
describe("beast-descriptors — pure helpers", () => {
  ...
  describe("buildNarrativeDescriptor", () => {
    it("uses name-flavor match for known families", () => {
      ...
      expect(
        buildNarrativeDescriptor(
          mockBeast({ slug: "wolf", name: "Wolf", size: "Medium" }),
        ),
      ).toMatch(/ágil|presas/);
      expect(
        buildNarrativeDescriptor(
    ...
    it("falls back to generic flavor when name is unknown", () => {
      const r = buildNarrativeDescriptor(
        mockBeast({
          slug: "exotic-foo",
          name: "Exotic Foo",
          size: "Small",
    ...
    it("respects 120-char max", () => {
      const r = buildNarrativeDescriptor(
        mockBeast({ name: "Ancient Armored Dinosaur of the Burning Canyons" }),
      );
      expect(r.length).toBeLessThanOrEqual(120);
    });
    it("localizes size adjective", () => {
      expect(buildNarrativeDescriptor(mockBeast({ size: "Large" }))).toContain(
        "grande",
      );
      expect(buildNarrativeDescriptor(mockBeast({ size: "Tiny" }))).toContain(
        "minúsculo",
      );
    });
  });
  ...
  describe("passesLocomotionFilter", () => {
    ...
    it("excludes fliers when excludeFly", () => {
      expect(passesLocomotionFilter(flyingBeast, { excludeFly: true })).toBe(
        false,
      );
      expect(passesLocomotionFilter(mockBeast(), { excludeFly: true })).toBe(
        true,
      );
    });
    it("excludes swimmers when excludeSwim", () => {
      expect(passesLocomotionFilter(swimmingBeast, { excludeSwim: true })).toBe(
        false,
      );
    });
    it("excludes burrowers when excludeBurrow", () => {
      expect(
        passesLocomotionFilter(burrowingBeast, { excludeBurrow: true }),
      ).toBe(false);
    });
  });
