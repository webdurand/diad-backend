...
export function extractSpeed(
  speed: SpeedLike | null | undefined,
): BeastSummary["speed"] {
...
function attackNames(actions: unknown, limit = 2): string[] {
  ...
  return actions
    ...
    .filter(
      (n): n is string => typeof n === "string" && !/multiattack/i.test(n),
    )
    .slice(0, limit);
}
...
const NAME_FLAVOR: ReadonlyArray<[RegExp, string]> = [
  ...
  [
    /lion|tiger|panther|leopard|cat/i,
    "felino ágil com garras retráteis e passos silenciosos",
  ],
  ...
  [
    /horse|pony|mule|donkey|elk|deer|stag|moose/i,
    "quadrúpede veloz de cascos firmes",
  ],
...
export function buildNarrativeDescriptor(monster: MonsterEntity): string {
  const sizeAdj =
    SIZE_ADJECTIVES[(monster.size || "").toLowerCase()] ?? monster.size;
  const lowerName = monster.name.toLowerCase();
  const flavor =
    NAME_FLAVOR.find(([re]) => re.test(lowerName))?.[1] ??
...
export function passesLocomotionFilter(
  ...
  if (filter.excludeBurrow && speed.burrow != null && speed.burrow > 0)
    return false;
  return true;
}
