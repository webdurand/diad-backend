...
const mockSheet = {
  ...
  skills: [
    { slug: 'athletics', name: 'Athletics', bonus: 5, proficient: true, expertise: false },
    { slug: 'stealth', name: 'Stealth', bonus: 4, proficient: true, expertise: false },
    { slug: 'perception', name: 'Perception', bonus: 3, proficient: true, expertise: false },
    { slug: 'persuasion', name: 'Persuasion', bonus: -1, proficient: false, expertise: false },
    { slug: 'acrobatics', name: 'Acrobatics', bonus: 6, proficient: true, expertise: true },
  ],
  conditions: [],
};
...
describe('SkillCheckService', () => {
  ...
  it('should fail when ability not found', async () => {
    const result = await service.rollAbilityCheck({ ...baseDto, ability: 'xyz' });
    expect(result.ok).toBe(false);
    if (result.ok) return;
  ...
  it('should handle manual advantage override', async () => {
    const result = await service.rollAbilityCheck({ ...baseDto, advantage: true });
    expect(result.ok).toBe(true);
    if (!result.ok) return;
    expect(result.value.advantage).toBeDefined();
