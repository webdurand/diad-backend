...
function makeParticipant(over: Partial<EncounterParticipantEntity> = {}): EncounterParticipantEntity {
  return {
    id: over.id ?? 'p1',
    type: over.type ?? 'pc',
...
describe('WeaponMasteryService', () => {
  ...
  let appliedConditions: Array<{ target: EncounterParticipantEntity; input: any }>;
  ...
  beforeEach(() => {
    ...
    const effectInstanceSvc: any = {
      addEffect: async (target: EncounterParticipantEntity, input: any) => {
        ...
        target.effectInstances = [...(target.effectInstances ?? []), effect as any];
        return { effect, events: [{ event_type: 'effect_applied', data: { kind: input.kind } }] };
      },
      removeEffect: async (target: EncounterParticipantEntity, id: string) => {
        removedEffects.push({ target, id });
        target.effectInstances = (target.effectInstances ?? []).filter((e) => e.id !== id);
        return { removed: true, events: [] };
      },
    };

    const conditionLifecycle: any = {
      applyCondition: async (target: EncounterParticipantEntity, input: any) => {
        appliedConditions.push({ target, input });
        target.conditions = [...(target.conditions ?? []), input.slug];
        return { events: [{ event_type: 'condition_applied', data: { slug: input.slug } }], instance: {}, concentrationBroken: false };
      },
    };
    ...
    service = new WeaponMasteryService(participantRepo, dice, effectInstanceSvc, conditionLifecycle);
  });
  ...
  describe('Topple (on hit)', () => {
    it('aplica prone quando save falha', async () => {
      ...
      const target = makeParticipant({
        id: 'p2',
        type: 'monster',
        monster: { constitution: 8, proficiency_bonus: 2, proficiencies: [] } as any,
      });
      const res = await service.resolveOnHit({
        masterySlug: 'topple',
        attacker: makeParticipant(),
    ...
    it('DC scale com profBonus e abilityMod', async () => {
      const target = makeParticipant({
        id: 'p2',
        type: 'monster',
        monster: { constitution: 16, proficiency_bonus: 3, proficiencies: [] } as any,
      });
      const res = await service.resolveOnHit({
        masterySlug: 'topple',
        attacker: makeParticipant(),
