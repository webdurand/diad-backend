...
const burningHands: MiniSpell = { slug: 'burning-hands', area_of_effect: { type: 'cone', size: 15 } };
const fireball: MiniSpell = { slug: 'fireball', area_of_effect: { type: 'sphere', size: 20 } };
...
const eldritchBlast: MiniSpell = { slug: 'eldritch-blast', area_of_effect: null };
...
describe('spell-targeting — US14 (Spec 005)', () => {
  describe('isAoeSpell', () => {
    ...
    it('rejeita area_of_effect XPHB que só tem tags (sem shape real)', () => {
      ...
      const xphbMultiTarget = { slug: 'acid-splash-phb', area_of_effect: { tags: ['multiple targets'] } };
      const xphbSingle = { slug: 'fire-bolt-phb', area_of_effect: { tags: ['single target'] } };
  ...
  describe('maxTargetsFor', () => {
    it('retorna Infinity para spells AoE (forma define; range validado à parte)', () => {
      expect(maxTargetsFor(burningHands as any, 1, 3)).toBe(Number.POSITIVE_INFINITY);
      expect(maxTargetsFor(fireball as any, 3, 5)).toBe(Number.POSITIVE_INFINITY);
    });
  ...
  describe('MULTI_TARGET_NON_AOE_SPELLS catálogo', () => {
    ...
    it('tolera sufixos de fonte (-phb, -xphb, etc.)', () => {
      ...
      const splashVariant: MiniSpell = { slug: 'acid-splash-phb', area_of_effect: null };
