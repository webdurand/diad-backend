...
function generateEventDescription(
  ...
  switch (eventType) {
    case 'attack_roll': {
      const weapon = (data.actionName as string) ?? (data.actionSlug as string) ?? 'ataque';
    ...
    case 'spell_cast': {
      const spell = (data.spellName as string) ?? (data.spellSlug as string) ?? 'magia';
      const slot = data.slotLevel ?? data.slotUsed ?? '';
      return `${actor} conjura ${spell}${slot ? ` (slot nível ${slot})` : ''}${targetName ? ` em ${target}` : ''}`;
    }
    case 'condition_applied': {
      const condition = (data.condition as string) ?? (data.slug as string) ?? '?';
    ...
    case 'effect_applied': {
      ...
      const source = (data.source as string) ?? (data.sourceSpell as string) ?? '';
    ...
    case 'movement': {
      const from = data.from ? `(${(data.from as any).x},${(data.from as any).y})` : '';
