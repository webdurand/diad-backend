...
export class BerserkerService {
  ...
  > {
    ...
    const hasFrenzy = (sheet as unknown as { hasFrenzy?: boolean }).hasFrenzy === true;
    if (!hasFrenzy) {
    ...
    return success({ damage, diceCount, targetPrevHp: prevHp, targetNewHp: target.currentHp }, [event]);
  }
  ...
  > {
    ...
    const sheet = await this.sheetService.computeSheet(userId, barbarian.characterId);
    const hasRetaliation = (sheet as unknown as { hasRetaliation?: boolean }).hasRetaliation === true;
    if (!hasRetaliation) {
      return failure('Requer Berserker L10+ com Retaliation.', 'FEATURE_NOT_AVAILABLE');
    }
    if ((barbarian.reactionsUsed ?? 0) >= 1) {
      return failure('Sem reactions disponíveis este turno.', 'NO_REACTIONS_REMAINING');
    }
    ...
    return success({ attackRoll, attackTotal, targetAc, hit, damage, targetPrevHp: prevHp, targetNewHp: newHp }, [event]);
  }
  ...
  > {
    ...
    const sheet = await this.sheetService.computeSheet(userId, barbarian.characterId);
    const hasIP = (sheet as unknown as { hasIntimidatingPresence?: boolean }).hasIntimidatingPresence === true;
    if (!hasIP) {
      return failure('Requer Berserker L14+ com Intimidating Presence.', 'FEATURE_NOT_AVAILABLE');
    }
  ...
  private getTargetAc(p: EncounterParticipantEntity): number {
    if (p.type === 'monster' && (p as unknown as { monster?: { armor_class?: Array<{ value: number }> } }).monster?.armor_class?.[0]) {
      return (p as unknown as { monster: { armor_class: Array<{ value: number }> } }).monster.armor_class[0].value;
    }
    return 10;
  }
