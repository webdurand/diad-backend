...
describe('InspirationService', () => {
  function setup(overrides: Partial<{ armed: boolean; type: 'pc' | 'monster'; characterId: string | null }> = {}) {
    const participant = {
      id: 'p1',
      inspirationArmed: overrides.armed ?? false,
      type: overrides.type ?? 'pc',
      characterId: overrides.characterId === undefined ? 'char-1' : overrides.characterId,
    };
  ...
  it('consumeIfArmed em monster → não chama setInspiration (não tem ficha)', async () => {
    const { svc, setInspiration } = setup({ armed: true, type: 'monster', characterId: null });
    const r = await svc.consumeIfArmed('p1', 'attack_roll');
    expect(r.consumed).toBe(true);
    expect(setInspiration).not.toHaveBeenCalled();
  });
  ...
  it('participant ausente → consumeIfArmed retorna consumed=false', async () => {
    ...
    const svc = new InspirationService({ findOne, save } as any, { setInspiration } as any);
