...
function buildSheet(overrides: Partial<CharacterSheet>): CharacterSheet {
  return {
    ...
    maxHp: 10, currentHp: 10, tempHp: 0, armorClass: 10, initiative: 0, speed: 30,
    hitDice: [], carryingCapacity: 0,
    deathSaves: { successes: 0, failures: 0 },
    skills: [], passivePerception: 10, savingThrows: [], proficiencies: [],
    features: [], spells: [], spellSlots: [],
    xp: 0,
    ...overrides,
  ...
  describe('getSpellcastingModifier', () => {
    it('retorna mod da ability de spellcasting (cleric/WIS 16 → +3)', () => {
      const sheet = buildSheet({
        classes: [
          {
            slug: 'cleric', name: 'Cleric', level: 1, hitDie: 8,
            spellcastingAbility: 'wis', spellSaveDc: 13, spellAttackBonus: 5,
          } as never,
        ],
      });
    ...
    it('retorna 0 quando não há caster class (Fighter puro)', () => {
      const sheet = buildSheet({
        classes: [{ slug: 'fighter', name: 'Fighter', level: 1, hitDie: 10 } as never],
      });
    ...
    it('escolhe primeira caster class em multi-class', () => {
      const sheet = buildSheet({
        classes: [
          ...
          {
            slug: 'wizard', name: 'Wizard', level: 3, hitDie: 6,
            spellcastingAbility: 'int', spellSaveDc: 13,
          } as never,
        ],
        abilityScores: [
          { slug: 'str', name: 'Strength', score: 10, modifier: 0 },
  ...
  describe('substituteSpellcastingMod', () => {
    const sheet = buildSheet({
      classes: [
        {
          slug: 'cleric', name: 'Cleric', level: 1, hitDie: 8,
          spellcastingAbility: 'wis', spellSaveDc: 13,
        } as never,
      ],
    });
    ...
    it('aceita mod negativo (STR -1 etc.)', () => {
      const weakSheet = buildSheet({
        classes: [{ slug: 'wizard', name: 'Wizard', level: 1, hitDie: 6,
          spellcastingAbility: 'int', spellSaveDc: 10 } as never],
        abilityScores: [
          { slug: 'str', name: 'Strength', score: 10, modifier: 0 },
          { slug: 'dex', name: 'Dexterity', score: 10, modifier: 0 },
          { slug: 'con', name: 'Constitution', score: 10, modifier: 0 },
      ...
      expect(substituteSpellcastingMod('1d4 + MOD', weakSheet)).toBe('1d4 + -1');
    });
  });
});
