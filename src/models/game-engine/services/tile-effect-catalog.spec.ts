...
describe('tile-effect-catalog', () => {
  ...
  describe('catalog invariants (Princípio X)', () => {
    ...
    it.each(slugs)('entry "%s" has narrative descriptor ≤120 chars PT-BR', (slug) => {
      const def = TILE_EFFECT_CATALOG[slug];
    ...
    it.each(slugs)('entry "%s" has tactical metadata in valid range', (slug) => {
      const def = TILE_EFFECT_CATALOG[slug];
      ...
      expect(['caster', 'allies', 'neutral']).toContain(def.tactical.beneficiaryFaction);
    });
  });
  ...
  describe('Wall of Fire (PHB 2024 p.353)', () => {
    ...
    it('has on-pass-through-wall 5d8 fire (no save)', () => {
      const passThrough = def.triggers.find((t) => t.kind === 'on-pass-through-wall');
      expect(passThrough).toBeDefined();
      expect(passThrough?.damage?.expressionPerSlot(4)).toBe('5d8');
    });

    it('has on-end-turn-adjacent 5d8 fire range:1', () => {
      const endTurn = def.triggers.find((t) => t.kind === 'on-end-turn-adjacent');
