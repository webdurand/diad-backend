...
describe('spell-damage-catalog', () => {
  ...
  describe('Fireball', () => {
    it('rolls 8d6 at slot 3', () => {
      expect(getSpellDamage('fireball', 3, 5)).toEqual({ expression: '8d6', type: 'fire' });
    });

    it('rolls 10d6 at slot 5 (+1d6 per slot above 3)', () => {
      expect(getSpellDamage('fireball', 5, 9)).toEqual({ expression: '10d6', type: 'fire' });
    });
  ...
  describe('Scorching Ray', () => {
    it('rolls 6d6 at slot 2 (3 rays × 2d6)', () => {
      expect(getSpellDamage('scorching-ray', 2, 5)).toEqual({ expression: '6d6', type: 'fire' });
    });

    it('rolls 10d6 at slot 4 (5 rays × 2d6)', () => {
      expect(getSpellDamage('scorching-ray', 4, 7)).toEqual({ expression: '10d6', type: 'fire' });
    });
  });

  describe('Cone of Cold / Chain Lightning / Disintegrate', () => {
    it('cone-of-cold 8d8 at slot 5', () => {
      expect(getSpellDamage('cone-of-cold', 5, 9)).toEqual({ expression: '8d8', type: 'cold' });
    });

    it('chain-lightning 10d8 at slot 6 (no scaling)', () => {
      expect(getSpellDamage('chain-lightning', 6, 11)).toEqual({ expression: '10d8', type: 'lightning' });
    });

    it('disintegrate 10d6+40 at slot 6', () => {
      expect(getSpellDamage('disintegrate', 6, 11)).toEqual({ expression: '10d6+40', type: 'force' });
    });

    it('disintegrate 13d6+40 at slot 7 (+3d6 per slot above)', () => {
      expect(getSpellDamage('disintegrate', 7, 13)).toEqual({ expression: '13d6+40', type: 'force' });
    });
  });
  ...
  describe('Cantrips', () => {
    it('fire-bolt scales 1d10 → 2d10 → 3d10 → 4d10 at levels 1/5/11/17', () => {
      expect(getSpellDamage('fire-bolt', 0, 1)).toEqual({ expression: '1d10', type: 'fire' });
      expect(getSpellDamage('fire-bolt', 0, 5)).toEqual({ expression: '2d10', type: 'fire' });
      expect(getSpellDamage('fire-bolt', 0, 11)).toEqual({ expression: '3d10', type: 'fire' });
      expect(getSpellDamage('fire-bolt', 0, 17)).toEqual({ expression: '4d10', type: 'fire' });
    });

    it('sacred-flame scales d8 per tier', () => {
      expect(getSpellDamage('sacred-flame', 0, 5)).toEqual({ expression: '2d8', type: 'radiant' });
    });
  });

  describe('case-insensitive + misses', () => {
    it('uppercase slug still resolves', () => {
      expect(getSpellDamage('MAGIC-MISSILE', 1, 1)).toEqual({ expression: '3d4+3', type: 'force' });
    });
