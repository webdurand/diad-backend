...
describe('stripTags', () => {
  ...
  describe('reference tags', () => {
    ...
    it('{@optfeature}', () => {
      expect(stripTags('{@optfeature Agonizing Blast|XPHB}')).toBe('Agonizing Blast');
    });
    ...
    it('{@classFeature}', () => {
      expect(stripTags('{@classFeature Spellcasting|Wizard|XPHB|1}')).toBe('Spellcasting');
    });

    it('{@subclassFeature}', () => {
      expect(stripTags('{@subclassFeature Channel Divinity|Cleric|XPHB|Life|XPHB|6}')).toBe('Channel Divinity');
    });
  });
  ...
  describe('2024 action tags', () => {
    ...
    it('{@actSaveFail text}', () => {
      expect(stripTags('{@actSaveFail The target takes damage}')).toBe('Failure: The target takes damage');
    });
    ...
    it('{@hitYourSpellAttack}', () => {
      expect(stripTags('{@hitYourSpellAttack text}')).toBe('your spell attack modifier');
    });
  });
  ...
  describe('mixed text with multiple tags', () => {
    it('handles text with inline tags', () => {
      const input = 'You cast {@spell Fireball|XPHB} dealing {@damage 8d6} fire damage.';
      expect(stripTags(input)).toBe('You cast Fireball dealing 8d6 fire damage.');
    });
