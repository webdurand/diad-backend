...
interface FiveToolsRace {
  ...
  size?: string[];
  speed?: number | { walk?: number; fly?: number | boolean; swim?: number | boolean; climb?: number | boolean; burrow?: number | boolean };
  darkvision?: number;
...
function resolveAbilityBonuses(ability?: Record<string, number>[]): TransformedAbilityBonus[] {
  ...
  for (const block of ability) {
    for (const [key, val] of Object.entries(block)) {
      ...
      result.push({
        ability_score: { slug: fullName, name: fullName.charAt(0).toUpperCase() + fullName.slice(1) },
        bonus: val,
      });
    }
  }
...
function resolveAbilityBonusOptions(ability?: Record<string, unknown>[]): Record<string, unknown> | null {
...
function loadRacesJson(): { race: FiveToolsRace[]; subrace: FiveToolsSubrace[] } {
  const filePath = path.resolve(process.cwd(), '../5etools-src/data/races.json');
  ...
  return subraces
    ...
    .map((sub) => {
      ...
      return {
        ...
        name: sub.name!,
        description: descParts.join(' ') || `${sub.name} ${parentName} subrace.`,
