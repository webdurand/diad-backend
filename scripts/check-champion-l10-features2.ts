...
ds.initialize().then(async () => {
  const rows = await ds.query(`SELECT f.slug FROM level_features lf JOIN features f ON f.id=lf.feature_id WHERE lf.level_id='ead22c87-0a87-4192-8b52-93d138000f95'`);
  console.log('Champion L10 level_features after migration:', rows);
  await ds.destroy();
}).catch(console.error);
